    ...
    <section class="dashboard-grid">
      <div class="content-card">
        ...
        <el-table :data="data.recent_transactions" style="width: 100%">
          ...
          <el-table-column label="物料变化" min-width="230"><template #default="{ row }"><div class="tx-lines"><span v-for="line in row.lines" :key="line.id" class="tx-line">{{ line.name }} <b :class="line.quantity_change > 0 ? 'number-positive' : 'number-negative'">{{ line.quantity_change > 0 ? '+' : '' }}{{ qty(line.quantity_change) }}</b></span></div></template></el-table-column>
      ...
      <div style="display:flex;flex-direction:column;gap:18px">
        ...
        <div class="content-card">
          <div class="card-head"><h3>库存预警</h3><router-link to="/inventory">库存详情</router-link></div>
