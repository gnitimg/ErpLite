...
async function load(silent = false) {
  ...
  try {
    rows.value = await api(`/api/operation-logs?${params}`)
  } catch (error: any) {
...
async function loadActions() {
  try {
    actions.value = await api("/api/operation-logs/actions")
  } catch {
    actions.value = []
  }
}
  ...
  <div class="erp-page">
    <div class="page-toolbar">
      <div class="toolbar-group list-actions">
        ...
        <el-badge :value="activeFilterCount" :hidden="!activeFilterCount" class="filter-badge">
    ...
    <el-drawer v-model="filterDrawer" title="筛选操作日志" size="min(420px, 92vw)">
      <el-form label-position="top">
        <el-form-item label="操作类型">
          <el-select v-model="filters.action" clearable placeholder="全部操作" style="width:100%">
