    ...
    <el-drawer v-model="workflowDrawer" class="order-workspace-drawer" size="min(1040px, 98vw)">
      <template #header>
        <div class="order-workspace-header">
          <div class="order-workspace-title"><strong>客单工作区</strong><span class="mono">{{ activeOrder?.order_no || '' }}</span><el-tag v-if="activeOrder" :type="statusMap[activeOrder.status]?.type as any" size="small">{{ statusMap[activeOrder.status]?.label || activeOrder.status }}</el-tag></div>
          <div v-if="activeOrder" class="order-workspace-actions">
            <el-button v-if="activeOrder.status === 'DRAFT'" type="primary" @click="action(activeOrder,'confirm')">确认并检查库存</el-button>
            <el-button v-if="activeOrder.status === 'WAITING_MATERIALS'" type="primary" @click="action(activeOrder,'prepare')">重新检查并生产</el-button>
            <el-button v-if="activeOrder.status === 'READY_TO_SHIP'" type="success" @click="action(activeOrder,'fulfill')">确认产品出库</el-button>
            <el-button v-if="!['FULFILLED','CANCELLED'].includes(activeOrder.status)" plain type="danger" @click="action(activeOrder,'cancel')">取消客单</el-button>
          </div>
        </div>
      </template>
      <div v-loading="workflowLoading" class="order-workspace-body">
        <el-tabs v-model="activeDetailTab" class="order-detail-tabs">
          <el-tab-pane name="overview">
            <template #label><span class="detail-tab-label">订单概况</span></template>
            <div v-if="activeOrder" class="detail-page">
              <div class="order-overview-strip"><div><span>当前状态</span><strong>{{ statusMap[activeOrder.status]?.label || activeOrder.status }}</strong></div><div><span>要求交期</span><strong>{{ activeOrder.required_date }}</strong></div><div><span>产品数量</span><strong>{{ productQty(orderedQuantity) }}</strong></div><div><span>订单金额</span><strong>{{ money(activeOrder.total_amount) }}</strong></div></div>
              <div class="detail-section-head"><strong>客户与交付信息</strong><span>客单建立时记录的信息</span></div>
              <el-descriptions :column="2" border><el-descriptions-item label="客户">{{ activeOrder.customer_name }}</el-descriptions-item><el-descriptions-item label="联系电话">{{ activeOrder.customer_phone || '-' }}</el-descriptions-item><el-descriptions-item label="订单日期">{{ activeOrder.order_date }}</el-descriptions-item><el-descriptions-item label="要求交期"><b>{{ activeOrder.required_date }}</b></el-descriptions-item><el-descriptions-item label="送货地址" :span="2">{{ activeOrder.customer_address || '-' }}</el-descriptions-item><el-descriptions-item label="备注" :span="2">{{ activeOrder.notes || '-' }}</el-descriptions-item></el-descriptions>
              <div v-if="workflow" class="detail-section-head"><strong>当前处理建议</strong><span>根据库存、交期和 BOM 自动判断</span></div>
              <div v-if="workflow" class="next-action-card"><div class="next-action-icon"><el-icon><Guide /></el-icon></div><div><span>下一步</span><strong>{{ workflowNextAction }}</strong></div><el-button text type="primary" @click="activeDetailTab = workflow.next_action === 'PURCHASE' || workflow.next_action === 'CONFIGURE_BOM' ? 'materials' : 'inventory'">查看相关信息</el-button></div>
            </div>
          </el-tab-pane>
          <el-tab-pane name="pricing">
            <template #label><span class="detail-tab-label">产品与价格 <el-badge :value="activeOrder?.items?.length || 0" /></span></template>
            <div v-if="activeOrder" class="detail-page">
              <div class="detail-section-head detail-section-head-first"><strong>成交明细</strong><span>参考价用于比价，本单价格为实际出库成交价</span></div>
              <el-table :data="activeOrder.items" border><el-table-column label="产品" min-width="210"><template #default="{ row }"><div class="sku-cell"><strong>{{ row.product_name }}</strong><span>{{ row.product_sku }}</span></div></template></el-table-column><el-table-column label="数量" width="90" align="right"><template #default="{ row }">{{ productQty(row.quantity) }}</template></el-table-column><el-table-column label="参考价" width="120" align="right"><template #default="{ row }">{{ money(row.reference_price) }}</template></el-table-column><el-table-column label="本单价格" width="125" align="right"><template #default="{ row }"><b>{{ money(row.unit_price) }}</b></template></el-table-column><el-table-column label="折扣" width="90" align="right"><template #default="{ row }"><el-tag :type="row.discount_rate < 100 ? 'warning' : 'info'" size="small" effect="plain">{{ row.discount_rate }}%</el-tag></template></el-table-column><el-table-column label="优惠" width="120" align="right"><template #default="{ row }">{{ money(row.discount_amount) }}</template></el-table-column><el-table-column label="小计" width="125" align="right"><template #default="{ row }"><b>{{ money(row.line_total) }}</b></template></el-table-column></el-table>
              <div class="order-price-total"><span>客单成交总额</span><strong>{{ money(activeOrder.total_amount) }}</strong></div>
            </div>
          </el-tab-pane>
          <el-tab-pane name="inventory">
            <template #label><span class="detail-tab-label">库存与交付</span></template>
            <div v-if="workflow" class="detail-page">
              <div class="detail-section-head detail-section-head-first"><strong>履约进度</strong><span>按照交期优先级自动预留产品库存</span></div>
              <div class="workflow-panel"><div class="workflow-steps"><div class="workflow-step active">1 接单</div><div class="workflow-step" :class="{ active: workflow.status !== 'DRAFT' }">2 检查产品库存</div><div class="workflow-step" :class="{ active: ['WAITING_MATERIALS','READY_TO_SHIP','FULFILLED'].includes(workflow.status) }">3 零件采购/备料</div><div class="workflow-step" :class="{ active: ['READY_TO_SHIP','FULFILLED'].includes(workflow.status) }">4 生产并预留</div><div class="workflow-step" :class="{ active: workflow.status === 'FULFILLED' }">5 产品出库</div></div><div class="workflow-summary"><el-tag :type="statusMap[workflow.status]?.type as any">{{ statusMap[workflow.status]?.label || workflow.status }}</el-tag><span>{{ workflowNextAction }}</span></div></div>
              ...
              <div class="detail-section-head"><strong>产品库存与预留</strong><span>交期相同的客单按下单时间排序</span></div>
              <el-table :data="workflow.product_lines" border empty-text="暂无产品库存信息"><el-table-column label="产品" min-width="220"><template #default="{ row }"><div class="sku-cell"><strong>{{ row.name }}</strong><span>{{ row.sku }}</span></div></template></el-table-column><el-table-column label="交期优先级" width="140" align="center"><template #default="{ row }"><el-tag :type="row.waiting_for_earlier_orders ? 'warning' : 'success'" size="small">第 {{ row.priority_rank }} / {{ row.priority_total }} 位</el-tag></template></el-table-column><el-table-column label="订单数量" width="120" align="right"><template #default="{ row }">{{ productQty(row.ordered_quantity) }}</template></el-table-column><el-table-column label="已预留" width="120" align="right"><template #default="{ row }"><b class="number-positive">{{ productQty(row.reserved_quantity) }}</b></template></el-table-column><el-table-column label="需生产" width="120" align="right"><template #default="{ row }"><b :class="row.production_required ? 'number-negative' : ''">{{ productQty(row.production_required) }}</b></template></el-table-column></el-table>
            </div>
          </el-tab-pane>
          <el-tab-pane name="materials">
            <template #label><span class="detail-tab-label">零件与备料 <el-badge v-if="workflow?.material_lines?.length" :value="workflow.material_lines.length" /></span></template>
            <div v-if="workflow" class="detail-page">
              <div class="detail-section-head detail-section-head-first"><strong>零件需求与采购</strong><span>按 BOM 汇总全部缺货产品所需零件</span></div>
              ...
              <el-alert v-else-if="workflow.next_action === 'CONFIGURE_BOM'" title="缺货产品尚未配置 BOM，请先前往产品与 BOM 页面补充零件组成。" type="error" :closable="false" show-icon />
              <el-table :data="workflow.material_lines" border empty-text="当前无需额外生产，或暂无 BOM 零件需求"><el-table-column label="零件" min-width="230"><template #default="{ row }"><div class="sku-cell"><strong>{{ row.name }}</strong><span>{{ row.sku }}</span></div></template></el-table-column><el-table-column label="备料方式" width="140"><template #default="{ row }"><el-tag :type="row.supply_mode === 'BUY_TO_ORDER' ? 'warning' : 'info'" size="small" effect="plain">{{ row.supply_mode === 'BUY_TO_ORDER' ? '按单采购' : '库存备料' }}</el-tag></template></el-table-column><el-table-column label="需要" width="120" align="right"><template #default="{ row }">{{ qty(row.required_quantity) }}</template></el-table-column><el-table-column label="现有" width="120" align="right"><template #default="{ row }">{{ qty(row.available_stock) }}</template></el-table-column><el-table-column label="缺口" width="120" align="right"><template #default="{ row }"><b :class="row.shortage_quantity ? 'number-negative' : 'number-positive'">{{ qty(row.shortage_quantity) }}</b></template></el-table-column></el-table>
            </div>
          </el-tab-pane>
        </el-tabs>
      </div>
    ...
    <el-drawer v-model="drawer" title="新建客户订单" size="min(760px, 96vw)">
      <el-form label-position="top">
        ...
        <el-table :data="form.items" border>
          <el-table-column label="产品" min-width="260"><template #default="{ row: line }"><el-select v-model="line.product_id" filterable placeholder="选择产品" style="width:100%" @change="productChanged(line)"><el-option v-for="product in products" :key="product.id" :label="`${product.sku} · ${product.name}（库存 ${productQty(product.stock_qty)}）`" :value="product.id" :disabled="form.items.some(x => x !== line && x.product_id === product.id)" /></el-select></template></el-table-column>
          <el-table-column label="数量" width="125"><template #default="{ row: line }"><el-input-number v-model="line.quantity" :min="1" :step="1" step-strictly :precision="0" style="width:100%" /></template></el-table-column>
