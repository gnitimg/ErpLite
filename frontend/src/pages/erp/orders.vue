...
async function load() {
  loading.value = true
  try { [rows.value, products.value] = await Promise.all([api(`/api/orders${status.value ? `?status=${status.value}` : ''}`), api('/api/products')]) }
...
<template>
  <div>
    <div class="page-toolbar">
      <div class="toolbar-group"><el-select v-model="status" clearable placeholder="全部状态" style="width:145px" @change="load"><el-option v-for="(meta, key) in statusMap" :key="key" :label="meta.label" :value="key" /></el-select><el-button @click="load"><el-icon><Refresh /></el-icon>刷新</el-button></div>
