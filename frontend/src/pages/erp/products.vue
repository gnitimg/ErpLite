...
async function load() {
  ...
  try { [rows.value, parts.value] = await Promise.all([api(`/api/products?${params}`), api('/api/parts')]) }
  catch (error: any) { ElMessage.error(error.message) }
  finally { loading.value = false }
}
function applyFilters() { filterDrawer.value = false; load() }
function resetFilters() { filters.stockStatus = ''; filters.bomStatus = ''; applyFilters() }
function openCreate() { editingId.value = null; Object.assign(form, emptyForm()); form.components.push({ part_id: undefined, quantity: 1 }); drawer.value = true }
function openEdit(row: any) {
  editingId.value = row.id
  Object.assign(form, { sku: row.sku, name: row.name, unit: row.unit, spec: row.spec, cost_price: row.cost_price, sale_price: row.sale_price, min_stock: row.min_stock, components: row.components.map((x: any) => ({ part_id: x.part_id, quantity: x.quantity })) })
  drawer.value = true
}
function addComponent() { form.components.push({ part_id: undefined, quantity: 1 }) }
function removeComponent(index: number) { form.components.splice(index, 1) }
async function save() {
...
async function remove(row: any) {
  try {
    ...
    ElMessage.success('产品已停用'); await load()
...
<template>
  <div class="erp-page">
    <ListToolbar v-model="keyword" placeholder="搜索产品编码或名称" :filter-count="activeFilterCount" :loading="loading" @search="load" @filter="filterDrawer=true" @refresh="load">
      <el-button type="primary" @click="openCreate"><el-icon><Plus /></el-icon>新建产品</el-button>
    </ListToolbar>
    <div class="content-card">
      <div class="card-head"><h3>产品与 BOM</h3><span>产品由零件清单定义组成</span></div>
    ...
    <el-drawer v-model="filterDrawer" title="筛选产品" size="min(420px, 92vw)">
      <el-form label-position="top">
        <el-form-item label="库存状态"><el-select v-model="filters.stockStatus" clearable placeholder="全部状态" style="width:100%"><el-option label="库存偏低" value="LOW" /><el-option label="库存正常" value="NORMAL" /></el-select></el-form-item>
        <el-form-item label="BOM 状态"><el-select v-model="filters.bomStatus" clearable placeholder="全部状态" style="width:100%"><el-option label="已配置 BOM" value="CONFIGURED" /><el-option label="未配置 BOM" value="EMPTY" /></el-select></el-form-item>
    ...
    <el-drawer v-model="drawer" :title="editingId ? '编辑产品与 BOM' : '新建产品与 BOM'" size="min(720px, 96vw)">
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
        <div class="form-grid">
          <el-form-item label="产品编码" prop="sku"><el-input v-model="form.sku" placeholder="例如 FG-CTRL-01" /></el-form-item>
          <el-form-item label="产品名称" prop="name"><el-input v-model="form.name" placeholder="请输入产品名称" /></el-form-item>
          <el-form-item label="规格型号"><el-input v-model="form.spec" placeholder="版本、尺寸等" /></el-form-item>
        ...
        <el-table :data="form.components" border>
          <el-table-column label="组成零件" min-width="260"><template #default="{ row }"><el-select v-model="row.part_id" filterable placeholder="选择零件" style="width:100%"><el-option v-for="part in parts" :key="part.id" :label="`${part.sku} · ${part.name}`" :value="part.id" :disabled="form.components.some(x => x !== row && x.part_id === part.id)" /></el-select></template></el-table-column>
          <el-table-column label="单台用量" width="170"><template #default="{ row }"><el-input-number v-model="row.quantity" :min="0.001" :precision="3" :controls="false" style="width:100%" /></template></el-table-column>
          <el-table-column width="65" align="center"><template #default="{ $index }"><el-button link type="danger" @click="removeComponent($index)"><el-icon><Delete /></el-icon></el-button></template></el-table-column>
        </el-table>
        ...
        <div class="drawer-footer"><el-button @click="drawer=false">取消</el-button><el-button type="primary" :loading="saving" @click="save">保存产品</el-button></div>
      </el-form>
    </el-drawer>
  </div>
</template>
