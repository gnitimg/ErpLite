  ...
  <div class="erp-page">
    ...
    <div class="content-card">
      ...
      <el-table v-loading="loading" :data="rows">
        ...
        <el-table-column label="备料方式" width="105">
          <template #default="{ row }">
            <el-tag :type="row.supply_mode === 'BUY_TO_ORDER' ? 'warning' : 'info'" effect="plain" size="small">
        ...
        <el-table-column label="当前库存" width="115" align="right">
          <template #default="{ row }">
            <b :class="row.low_stock ? 'number-negative' : ''">{{ qty(row.stock_qty) }}</b> {{ row.unit }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="状态" width="85">
          <template #default="{ row }">
            <el-tag :type="row.low_stock ? 'danger' : 'success'" effect="light" size="small">
      ...
      <el-form label-position="top">
        <el-form-item label="备料方式">
          <el-select v-model="filters.supplyMode" clearable placeholder="全部方式" style="width:100%">
            <el-option label="库存备料" value="STOCK" /><el-option label="按单即买" value="BUY_TO_ORDER" />
          </el-select>
        </el-form-item>
        <el-form-item label="库存状态">
          <el-select v-model="filters.stockStatus" clearable placeholder="全部状态" style="width:100%">
            <el-option label="需要补货" value="LOW" /><el-option label="库存正常" value="NORMAL" />
          </el-select>
        </el-form-item>
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
        <div class="form-grid">
          ...
          <el-form-item label="规格型号">
            <el-input v-model="form.spec" placeholder="尺寸、型号等" />
          </el-form-item>
          <el-form-item label="计量单位">
            <el-input v-model="form.unit" placeholder="件 / 个 / 米" />
          </el-form-item>
          <el-form-item label="参考成本">
            <el-input-number v-model="form.cost_price" :min="0" :precision="2" :controls="false" style="width:100%" />
          </el-form-item>
          <el-form-item label="安全库存">
            <el-input-number v-model="form.min_stock" :min="0" :precision="2" :controls="false" style="width:100%" />
          </el-form-item>
          <el-form-item class="span-2" label="备料方式">
            <el-radio-group v-model="form.supply_mode">
              <el-radio-button value="STOCK">
                库存备料
              </el-radio-button><el-radio-button value="BUY_TO_ORDER">
                按单即买
              </el-radio-button>
