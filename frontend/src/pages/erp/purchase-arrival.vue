...
const form = reactive({ part_id: 0, supplier: "", quantity: 0, expected_arrival_at: "", notes: "" })
...
function applyFilter() {
  ...
  filteredRows.value = rows.value.filter(r => !token || `${r.commitment_no} ${r.part_sku} ${r.part_name} ${r.supplier}`.toLowerCase().includes(token))
}
...
async function markArrived(row: Commitment) {
  try {
    await ElMessageBox.confirm(`确认标记 ${row.commitment_no} 已到货？`, "到货确认", { type: "warning" })
...
async function remove(row: Commitment) {
  try {
    await ElMessageBox.confirm(`确认删除采购承诺 ${row.commitment_no}？`, "删除", { type: "warning" })
...
<template>
  <div class="erp-page">
    <ListToolbar v-model="keyword" placeholder="搜索承诺编号、零件、供应商" :loading="loading" @refresh="load" @update:model-value="applyFilter">
      <el-button type="primary" @click="dialog = true; form.part_id = 0; form.supplier = ''; form.quantity = 0; form.expected_arrival_at = ''; form.notes = ''">
    ...
    <div class="content-card">
      ...
      <el-table v-loading="loading" :data="filteredRows" row-key="id" empty-text="暂无采购承诺">
        <el-table-column label="承诺编号" prop="commitment_no" width="160" />
    ...
    <el-dialog v-model="dialog" title="新增采购到货承诺" width="480px">
      <el-form label-position="top">
        ...
        <el-form-item label="预计到货日期"><el-date-picker v-model="form.expected_arrival_at" type="date" value-format="YYYY-MM-DD" style="width:100%" /></el-form-item>
