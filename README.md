...
- 模具数量直接绑定产品并在产品目录维护；“生产能力”页面按产品维护单线日产量和安全系数，生产线总数统一在“系统管理 → 生产设置”维护。
...
备份覆盖零件/产品、产品模具数、BOM、客单、库存结存、库存流水、成品预留、生产设置、生产能力、生产批次及订单产量分配，文件默认保存在项目根目录的 `backups\`，该目录已加入 `.gitignore`。备份格式版本为 7；旧版 1–6 快照恢复时会自动补齐新增结构。备份文件含业务数据，请按敏感数据管理，不要提交到 Git 仓库。
...
| 生产设置 | `GET/PUT /api/system/production-settings` 统一维护并行生产线数量；生产位仅用于排产计算，不维护编号或名称 |
| 生产能力 | 产品接口通过 `mold_count` 保存模具数；`GET/POST/PUT/DELETE /api/production/capabilities` 按产品维护单线日产能力 |
