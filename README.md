...
- 默认侧栏按业务发生顺序组织为“销售 → 采购 → 生产 → 库存 → 出入库 → 资料与设置”。轻量应收、收款与客户贷项并入“销售 → 应收与收款”，物料主数据并入“资料与设置 → 物料资料”；原财务和基础资料地址保留兼容跳转。“资料与设置 → 业务规则”集中展示标准流程、单号规则和角色权限；每个账号还可拖动调整一级、二级菜单顺序并隐藏非固定一级入口，配置会随账号保存。
- 客户订单、库存单据、生产批次、外协、退货、应收、收款和客户抵扣均使用数据库事务内递增的可配置单号。管理员可设置前缀、数字位数和下一待用号；已有单据不改号，同前缀的待用号不能回退。
