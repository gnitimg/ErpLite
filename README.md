...
备份覆盖零件/产品、BOM、客单、库存结存、库存流水、成品预留、生产设置、生产批次、订单产量分配及历史单据快照，文件默认保存在项目根目录的 `backups\`。备份格式版本为 11；旧版 1–10 快照恢复时会自动补齐已出库数量、生产批次关联和单据快照字段。备份文件含业务数据，请按敏感数据管理，不要提交到 Git 仓库。
