...
- 客单确认后自动预留可承诺库存，并按全局生产位数量、产品模具数量、有效日产能力和现有生产占用模拟预计完成时间。
...
- 模具数量直接绑定产品并在产品目录维护；“生产能力”页面按产品维护单日单机产量和安全系数，生产线总数统一在“系统管理 → 生产设置”维护。
...
- 产品模具数、生产批次、生产分配和成品预留数量均使用整数；日产安全系数与 ETA 时间计算保留小数精度。
...
备份覆盖零件/产品、产品模具数、BOM、客单、库存结存、库存流水、成品预留、生产设置、生产能力、人工排期、生产批次及订单产量分配，文件默认保存在项目根目录的 `backups\`，该目录已加入 `.gitignore`。备份格式版本为 8；旧版 1–7 快照恢复时会自动补齐新增结构。备份文件含业务数据，请按敏感数据管理，不要提交到 Git 仓库。

### 客单库存流程

1. 新建客单时填写要求交期；产品成交价默认等于产品参考价，但可按本单修改。
2. 确认客单后，系统按要求交期、订单日期、客单创建顺序分配现有产品库存并记录预留。
3. 产品不足的部分按产品汇总为待生产需求；系统读取全局生产位数量、产品模具数量、有效日产能力和正在生产的批次，按 FCFS + 最早完成时间生成生产计划。
...
5. 每个产品由用户填写单日单机产量；时间块时长按 `生产数量 ÷（单日单机产量 × 安全系数）` 计算，默认安全系数建议为 0.85。
6. 系统建议排期可在“订单排产”时间轴中拖动确认。人工确认批次会锁定生产位、模具位和时间段，后续自动重算只调整未确认批次。
...
| 生产能力 | 产品接口通过 `mold_count` 保存模具数；`GET/POST/PUT/DELETE /api/production/capabilities` 按产品维护单日单机产量 |
| 生产计划与排期 | `GET /api/production/runs`、`POST /api/production/plan/recalculate`、`PUT/DELETE /api/production/runs/{id}/schedule`、`PUT /api/production/runs/{id}/status` |
