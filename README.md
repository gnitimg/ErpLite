...
4. 批次完工审核在同一事务中按实际合格数量扣减 BOM。普通产品直接增加成品；配置外协的产品增加半成品，外协回厂后才增加成品并重新分配预留。半成品和外协在途会抵扣生产需求，不会重复排产。
...
5. 到“业务处理 → 生产入库”审核待完工批次，可修改实际合格数量并填写完成日期和备注；审核后 BOM 零件自动出库。普通产品进入成品库存；配置外协工序的产品进入半成品库存，再从“业务处理 → 外协加工”办理送出和回厂。
