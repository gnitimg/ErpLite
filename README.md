...
- “资料与设置 → 物料资料”统一维护材料和产品：使用页内分段开关快速切换数据、筛选和编辑表单；材料维护编码、规格、单位、成本、安全库存与备料方式，产品维护 BOM、价格、模具数、单机日产量及外协属性。原产品/材料链接会自动跳转到统一页面并保留对应范围。
- “库存 → 库存总览”使用同样的紧凑分段开关切换原料与成品库存，切换时同步更新数据、缺口口径和地址参数。
- 待购买按全部生产缺口全局汇总；排产时直接标记批次缺料明细，开始生产前再次强校验，并在开工时自动生成 BOM 零件出库单。完工审核只确认实际合格数和成品/半成品入库。
- 零件或产品手工出库，严格禁止负库存。
- 客户订单：草稿、确认、部分出库、整单出库和完成；首次出库后冻结订单业务内容。
- 取消客单时若关联了已人工排定或正在生产的批次，页面会列出相关计划并要求选择：取消未开工计划、转为自主补库存，或仅解除客单关联；已发生出库的客单禁止直接取消，应通过退货流程处理。
...
- “出入库 → 库存单据”独立汇总入库单和出库单；“开入库单/开出库单”以弹窗复用同一套多行单据，可混合选择零件和产品，编辑往来单位、日期、数量与单价，并支持逐行清空和全部清空。从客户订单进入时自动回填可编辑的出库内容。提交成功后弹窗展示数据库生成的正式单号，并提供“打印入库单/打印出库单”；未提交的草稿不提供打印入口，避免打印没有编号的单据。
...
- “生产排期 → 排产看板”提供按日期显示的时间轴；生产位行数取自全局产线数，可将待确认产品批次拖到任意生产位的空闲日期，冲突校验通过后锁定人工排期并更新客单 ETA。任务块采用双层受控指针拖动，松手时按任务块左边缘对应的日期落位。时间轴没有固定前后日期边界：接近边缘时自动扩展；在图内按住鼠标右键拖动可同时平移日期和生产位视图，原有横向、纵向滚动条继续保留。
...
- 默认侧栏按业务发生顺序组织为“销售 → 采购 → 生产 → 库存 → 出入库 → 资料与设置”。轻量应收、收款与客户贷项并入“销售 → 应收与收款”，物料主数据并入“资料与设置 → 物料资料”；原财务和基础资料地址保留兼容跳转。“资料与设置 → 业务规则”集中维护单号规则、展示角色权限边界并调整个人侧栏布局；每个账号可拖动一级、二级菜单并隐藏非固定入口，配置会随账号保存。
- 客户订单、库存单据、生产批次、外协、退货、应收、收款和客户抵扣均使用数据库事务内递增的可配置单号。管理员可设置前缀、数字位数和下一待用号，待用号码允许为 0 或重新指定；已有单据不改号，实际生成时自动跳过已经使用的编号。
- 权限分为只读、操作员和管理员：只读只能查询；操作员可执行业务写入；管理员另可维护用户、备份、全局参数和编号规则。权限由后端接口统一校验，前端说明或隐藏入口不作为安全边界。

## 数据链路与一致性

当前已打通的核心业务链路如下：

1. 在“资料与设置 → 物料资料”切换到产品并保存 BOM；材料和产品共用统一物料入口，但仍按各自数据结构和接口维护。
...
备份覆盖零件/产品、BOM、客单、退货、CustomerCredit、出库履约分配、成品/半成品/外协在途库存、外协批次、库存流水、成品预留、生产设置、单号规则、个人侧栏设置、生产批次、订单产量分配及历史单据快照，文件默认保存在项目根目录的 `backups\`。备份格式版本为 23；旧版 1–22 快照恢复时会自动补齐兼容字段和空表，但旧退货 snapshot 无法凭空恢复。备份文件含业务数据，请按敏感数据管理，不要提交到 Git 仓库。
...
| 业务规则 | `GET/PUT /api/system/document-numbering` 维护八类业务单号；`GET /api/system/role-permissions` 查看角色边界；`PUT /api/v1/users/me/navigation` 保存当前账号的侧栏顺序与显示项 |
